...
contract DugiStakingFixedPoolB is Ownable {
    ...
    uint256 public constant LOCKING_PERIOD_FOUR_YEARS = 4 * 365 * 24 * 60 * 60; // 4 years in seconds

    uint256 public constant REWARD_RATE_FOR_FOUR_YEAR =  4*15 * 10**16; //15% annual rate total of (15*4) 60% of staked token after 4  years
    ...
    function stakeForFourYears(uint256 amount) external hasMinimumStake(amount) {
        ...
        uint256 reward = LOCKING_PERIOD_FOUR_YEARS * REWARD_RATE_FOR_FOUR_YEAR * amount ;
    ...
    function unstakeFromFourYears(uint256 index) external {
        Stake memory userStake = stakes[msg.sender][index];
        require(userStake.amount > 0, "No staked amount");
        require(block.timestamp >= userStake.startTime + LOCKING_PERIOD_FOUR_YEARS, "Locking period not over");
