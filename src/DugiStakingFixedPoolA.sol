...
contract DugiStakingFixedPoolA is Ownable {
    IERC20 public token;
    ...
    uint256 public constant LOCKING_PERIOD_TWO_YEARS = 2 * 365 * 24 * 60 * 60; // 2 years in seconds
    uint256 public constant REWARD_RATE_FOR_TWO_YEAR = 2 * 10 * 10**16; // 10% annual rate total of 20% of staked token after 2 years
    ...
    function stakeForTwoYears(uint256 amount) external hasMinimumStake(amount) {
        ...
        uint256 reward = LOCKING_PERIOD_TWO_YEARS * REWARD_RATE_FOR_TWO_YEAR * amount;
    ...
    function unstakeFromTwoYears(uint256 index) external {
        Stake memory userStake = stakes[msg.sender][index];
        require(userStake.amount > 0, "No staked amount");
        require(block.timestamp >= userStake.startTime + LOCKING_PERIOD_TWO_YEARS, "Locking period not over");
