...
contract DugiStakingFixedPoolC is Ownable {
    ...
    uint256 public constant LOCKING_PERIOD_SIX_YEARS = 6 * 365 * 24 * 60 * 60; // 6 years in seconds

    uint256 public constant REWARD_RATE_FOR_SIX_YEAR =  6*20 * 10**16; // 20% annual rate total of (20*6) 120% of staked token after 6 years
 ...
 // mapping(address => mapping(uint256 => uint256)) public balances;
    ...
    modifier hasMinimumStake(uint256 amount) {
        ...
        function stakeForSixYears(uint256 amount) external hasMinimumStake(amount) {
        ...
        uint256 reward = LOCKING_PERIOD_SIX_YEARS * REWARD_RATE_FOR_SIX_YEAR * amount ;
    ...
    function unstakeFromSixYears(uint256 index) external {
        Stake memory userStake = stakes[msg.sender][index];
        require(userStake.amount > 0, "No staked amount");
        require(block.timestamp >= userStake.startTime + LOCKING_PERIOD_SIX_YEARS, "Locking period not over");
